...
int main(){
  ...
  world->newFallBody(btVector3(rick->LX, rick->LY, rick->LZ), btVector3(0, 10, 0), 1); //1
  
  /* PAREDES */
  world->newFallBody(btVector3(100, 100, 0.5), btVector3(0, 0, -2), 0); //2
  world->newFallBody(btVector3(100, 100, 0.5), btVector3(0, 0, 7), 0); //3
  ...
  for (int i = 0; i < 18; i+=1) {
    ...
    world->newFallBody(btVector3(plataformas[i]->LX / 2, plataformas[i]->LY / 2 + 0.1, plataformas[i]->LZ / 2), btVector3(i * plataformas[i]->LX, (i % 3) * 3, (i % 3) * plataformas[i]->LZ), 0);
    //4 - 21
  }
  ...
  while (!glfwWindowShouldClose(g_window)){
    ...
    if (glfwGetTime() >= 5.0f && firstTime) {
      world->setGravity(1, btVector3(1.5,-9.8,0));
      firstTime = false;
    } 
    window_frameCounter();
    ...
    // plataforma->draw();
    int rePos = (int)(rickPos.getX() / 100) * 100;
    if (rePos > lastRePos)
    {
      cout << "repos realizado" << endl;
      /* PAREDES */
      world->newFallBody(btVector3(100, 100, 0.5), btVector3(rePos, 0, -2), 0); //2
      world->newFallBody(btVector3(100, 100, 0.5), btVector3(rePos, 0, 7), 0);  //3
      for (int i = 0; i < 18; i += 1)
      {
        plataformas[i]->setpos(glm::vec3((i * plataformas[i]->LX) + rePos, (i % 3) * 3, (i % 3) * plataformas[i]->LZ));
        plataformas[i]->scale(glm::vec3(1.0f));
        plataformas[i]->setColor(0.753f, 0.753f, 0.753f);
        plataformas[i]->model2shader(shader_programme);
        world->newFallBody(btVector3(plataformas[i]->LX / 2, plataformas[i]->LY / 2 + 0.1, plataformas[i]->LZ / 2), btVector3((i * plataformas[i]->LX) + rePos, (i % 3) * 3, (i % 3) * plataformas[i]->LZ), 0);
      }
      lastRePos = rePos;
    }

    for (int i = 0; i < 18; i += 1) {
      // platPos = plataformas[i]->getpos();
      // plataformas[i]->setpos(glm::vec3(platPos.x + rePos, platPos.y, platPos.z));
      plataformas[i]->draw();
    }
    ...
    // for (float i = 0; i <= 100; i += 1.05)
    // {
    //   for (float j = 0; j <= 100; j += 1.05)
    //   {
    //     pared->setpos(glm::vec3(rickPos.getX() + i, -5, rickPos.getZ() + j));
    //     pared->draw();
    //   }
    // }
    pared->setpos(glm::vec3(50 + rePos, 50, -2));
    pared->draw();

    pared->setpos(glm::vec3(50 + rePos, 50, 7));
    pared->draw();
